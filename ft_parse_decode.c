...
/*   Updated: 2022/05/09 17:12:48 by pguranda         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	parse_args(const char *s, size_t z, lst_arg* out)
{
	...
	str_arg = malloc(arg_len * sizeof(char) + 1);
	if (str_arg == NULL)
		return ;
...
void ft_decode_to_struct(char *str_arg, size_t i, lst_arg* out)
{
	...
	counter = 0;
	address_dot = malloc(sizeof(char *));
	if (address_dot == NULL)
		return ;
	*address_dot = '0';
	...
	while (counter != i)
	{
		...
		if ((digit(str_arg[counter]) == 1 && &str_arg[counter] < address_dot) || *address_dot == '0')
